...
// Type definitions for molecule generation data and history
interface GeneratedMolecule {
  structure: string;
  score: number;
}

interface MoleculeGenerationHistory {
  smiles: string;
  numMolecules: number;
...
const ModalLayout = () => {
  ...
  const [numMolecules, setNumMolecules] = useState<string>("10");
  const [minSimilarity, setMinSimilarity] = useState<string>("0.3");
  const [particles, setParticles] = useState<string>("30");
  const [iterations, setIterations] = useState<string>("10");
  const [molecules, setMolecules] = useState<GeneratedMolecule[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [history, setHistory] = useState<MoleculeGenerationHistory[]>([]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/proxy/molecule-generation", {
            ...
            <div className="mt-4 max-h-96 overflow-y-auto">
              {history.map((entry, index) => (
                <div key={index} className="border-b border-stroke py-3">
                  ...
                  <p className="text-sm text-black dark:text-white">
                    <span className="font-bold">Molecules:</span> {entry.numMolecules}
                  </p>
                  <p className="text-sm text-black dark:text-white">
